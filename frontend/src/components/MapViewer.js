...
function HotspotButton({ spot, isActive, onClick, language }) {
  const title = localizeField(spot.title, language);
  const block = localizeField(spot.block, language);
  const floorLabel = localizeField(spot.floorLabel, language);
  ...
  return (
    <button
      ...
      aria-label={`${title} — ${block}, ${floorLabel}`}
...
function MapViewer({ layer, hotspots, allHotspots, selectedHotspotId, onHotspotSelect, routePoints, transformRef, layers, onLayerChange }) {
  ...
  const layerName = localizeLayerName(layer, language);
  ...
  return (
    <div
      ...
      role="application"
      aria-label={`${t("interactiveMap")} — ${layerName}`}
    >
      <TransformWrapper
        ref={transformRef}
        minScale={0.3}
          ...
          <>
            ...
            <TransformComponent wrapperClass="h-full w-full" contentClass="h-full w-full">
              <div className="relative h-screen w-screen select-none">
                ...
                <div className="absolute inset-0" role="group" aria-label={t("pointsOfInterest")}>
                  {hotspots.map((spot) => (
                    <MemoizedHotspot
                      key={spot.id}
                      spot={spot}
